...
def n_iterations(df_train, window_len):
    '''
    Calculate the number of steps needed, dependent on window size input
...
def walk_forward_validation(df_train, df_sent_train, config, window_len):
    '''
    Calculates the RMSE for a single SARIMA configuration
    '''
...
def sarimax_configs(seasonal=[0]):
    '''
    ...
    #t_params = ['n', 'c', 't', 'ct']
    t_params = ['n']
    P_params = np.arange(start=0, stop=0 + 1, step=1)
    D_params = np.arange(start=0, stop=0 + 1, step=1)
    Q_params = np.arange(start=0, stop=0 + 1, step=1)
    m_params = seasonal
...
if __name__ == '__main__':
    ...
    df_train.index[window_len]
    prediction_dates = df_train.iloc[window_len:].index
